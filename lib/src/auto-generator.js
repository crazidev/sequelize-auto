...
class AutoGenerator {
    ...
    makeHeaderImportV7() {
        ...
        header += "import { Attribute, PrimaryKey, AutoIncrement, ColumnName, NotNull, Default, Table, Unique, Comment, BelongsToMany, BelongsTo, HasMany, HasOne } from '@sequelize/core/decorators-legacy';\n\n";
        return header;
    }
    generateText() {
    ...
    generateTabelOptionV7(table) {
        const [, tableNameOrig] = (0, _types.qNameSplit)(table);
            ...
            if (!rel.isM2M) {
                if (rel.childTable === table) {
                    ...
                    if (this.options.version === 'v7') {
                        str += sp + `@BelongsTo(() => ${rel.parentModel}, { foreignKey: '${rel.parentId}', inverse: { as: '${rel.parentProp}', type: '${rel.isOne ? 'hasOne' : 'hasMany'}' }})\n`;
                    ...
                    if (rel.isOne) {
                        ...
                        if (this.options.version === 'v7') {
                            str += sp + `@HasOne(() => ${rel.childModel}, { foreignKey: '${rel.parentId}', inverse: { as: '${rel.childProp}' }})\n`;
                        ...
                        if (this.options.version === 'v7') {
                            str += sp + `@HasMany(() => ${rel.childModel}, { foreignKey: '${rel.parentId}', inverse: { as: '${rel.childProp}' } })\n`;
                            str += sp + `declare ${rel.childProp}?: NonAttribute<${rel.childModel}[]>\n\n`;
                        } else {
