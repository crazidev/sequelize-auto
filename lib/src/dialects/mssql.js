...
Object.defineProperty(exports, "mssqlOptions", {
    enumerable: true,
    get: function() {
        return mssqlOptions;
    }
...
const mssqlOptions = {
   ...
   */ getForeignKeysQuery: (tableName, schemaName)=>{
        return `SELECT ccu.TABLE_NAME AS source_table,
             ...
             WHERE tc.CONSTRAINT_TYPE != 'CHECK'
               AND ccu.TABLE_NAME = ${(0, _dialectoptions.addTicks)(tableName)}
                   ${(0, _dialectoptions.makeCondition)('ccu.TABLE_SCHEMA', schemaName)}`;
    },
    /**
   ...
   */ countTriggerQuery: (tableName, schemaName)=>{
        ...
        const qname = (0, _dialectoptions.addTicks)((schemaName ? schemaName + "." : "") + tableName);
   ...
   * @return {Bool}
   */ isSerialKey: (record)=>{
        return _lodash.default.isObject(record) && mssqlOptions.isPrimaryKey(record) && _lodash.default.has(record, 'is_identity') && record.is_identity;
    },
    /**
   ...
   */ showTablesQuery: (schemaName)=>{
    ...
    /** Sequelize "describeTable" doesn't include precision and scale in mssql */ showPrecisionQuery: (tableName, schemaName)=>{
