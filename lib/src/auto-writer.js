...
class AutoWriter {
    ...
    write() {
        ...
        const tablesM = lodash_1.default.keys(this.tableText);
    ...
    generateFkFile(tableNames) {
        ...
        tableNames.forEach((table) => {
            if (this.foreignKeys[table]) {
                ...
                lodash_1.default.keys(foreignKeys).forEach((field) => {
        ...
        str += `${this.space[1]}}\n`;
        str += `};\n`;
        const initFkFilePath = path_1.default.join(this.options.directory, 'migrations', 'zzz_add_constraints.js');
        const writeFile = util_1.default.promisify(fs_1.default.writeFile);
        const initFkPromise = writeFile(path_1.default.resolve(initFkFilePath), str);
    ...
    createFile(table, index) {
        ...
        const writeFile = util_1.default.promisify(fs_1.default.writeFile);
        if (this.options.generateMigration) {
            const migrationsDir = path_1.default.join(this.options.directory, 'migrations');
            if (!fs_1.default.existsSync(migrationsDir)) {
                mkdirp.sync(migrationsDir);
            }
            const fileMigrationPath = path_1.default.join(migrationsDir, `create-${fileName}-table.js`);
            writeFile(path_1.default.resolve(fileMigrationPath), this.tableMigration[table]);
        }
